...
# Filter auslesen
FILTER=$(echo "$QUERY_STRING" | sed -n 's/.*filter=\([^&]*\).*/\1/p')
...
head -n 1 "$DATEI_PFAD" | awk -F';' -v filter="$FILTER" '{
    print "<tr>";
    if (filter=="frauen") {
        print "<th>Jahr</th><th>Monat</th><th>Woche</th><th>Wochenstart</th><th>Frauen</th><th>Total</th>"
    } else if (filter=="maenner") {
        print "<th>Jahr</th><th>Monat</th><th>Woche</th><th>Wochenstart</th><th>Männer</th><th>Total</th>"
    } else {
